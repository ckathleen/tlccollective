<template>
  <div>
  <b>ABOUT</b><br>
    Clubhouse is an audio-chat platform founded by <a href="https://twitter.com/pdavison">Paul Davison</a> and <a href="https://twitter.com/rohanseth">Rohan Seth</a>.
    <br/><br/>
    <b>TLC'S INVESTMENT</b><br>
    Lauren and Casey invested in this company in summer 2020.
    <br><br>
    <b>CLUBHOUSE IN THE NEWS</b>
    <!--<iframe
      src="https://docs.google.com/document/d/e/2PACX-1vR3flcVH0GDEJE4U7NonIz63gMHDBoKGNf9zqgN5yjfyOcJ6ONALFhScfEQybZyGp7FUxx96xxwnH4h/pub?embedded=true"
      width="487"
      and
      height="360"
    ></iframe>-->
    We don't have a public  memo available for this investment.
    <br/><br/>
    Clubhouse in the news:
    <br/>
    <ul>
      <li>--> <u><a href="https://www.cnbc.com/2020/05/20/clubhouse-app-is-where-mc-hammer-and-jared-leto-chat-with-vcs.html" target="_blank">Silicon Valley is going crazy for Clubhouse</a></u></li>
      <li>--> <u><a href="https://www.nytimes.com/2020/05/19/technology/clubby-silicon-valley-app-clubhouse.html" target="_blank">The Hot New Thing in Clubby Silicon Valley? An App Called Clubhouse</a></u></li>
      <li>--> <u><a href="https://www.wired.com/story/what-is-clubhouse-why-does-silicon-valley-care/" target="_blank">What Is Clubhouse, and Why Does Silicon Valley Care?</a></u></li>
    </ul>
  </div>
</template>

<script>
export default {
  name: 'FolderContentPortfolioClubhouse'
}
</script>

<style lang="scss" scoped></style>
